/**
 * Extract allies/enemies from Mattel *Alies.jsx / *Enimies.jsx, write src/data/mattel/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-mattel-json.mjs
 */
import fs from 'fs';
...
const franchiseRoot = path.join(root, 'src', 'components', 'Mattel');
const dataDir = path.join(root, 'src', 'data', 'mattel');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/mattel/")) {
    c = c.replace(
...
for (const folder of franchises) {
  ...
  const importLine = `import mdata from '../../../data/mattel/${jsonName}';`;
