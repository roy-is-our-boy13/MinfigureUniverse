/**
 * Extract allies/enemies from Lucasfilm *Alies.jsx / *Enimies.jsx, write src/data/lucasfilm/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-lucasfilm-json.mjs
 */
import fs from 'fs';
...
const franchiseRoot = path.join(root, 'src', 'components', 'Lucasfilm');
const dataDir = path.join(root, 'src', 'data', 'lucasfilm');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/lucasfilm/")) {
    c = c.replace(
...
for (const folder of franchises) {
  ...
  const importLine = `import mdata from '../../../data/lucasfilm/${jsonName}';`;
