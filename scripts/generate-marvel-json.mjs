/**
 * Extract allies/enemies from Marvel *Alies.jsx / *Enimies.jsx, write src/data/marvel/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-marvel-json.mjs
 */
import fs from 'fs';
...
const marvelDir = path.join(root, 'src', 'components', 'Marvel');
const dataDir = path.join(root, 'src', 'data', 'marvel');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/marvel/")) {
    c = c.replace(
  ...
  // Remove const allies = [...]; or const enemies = [...];
...
const franchises = fs
  .readdirSync(marvelDir, { withFileTypes: true })
  .filter((d) => d.isDirectory())
  .map((d) => d.name);

for (const folder of franchises) {
  if (folder === 'SpiderMan') continue; // uses marvel/SpiderMan.json with family + allies + enemies

  const base = path.join(marvelDir, folder);
  ...
  const jsonName = `${pascalFromFolder(folder)}.json`;
  ...
  const importLine = `import mdata from '../../../data/marvel/${jsonName}';`;
