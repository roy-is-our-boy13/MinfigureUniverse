/**
 * Extract allies/enemies from Image *Alies.jsx / *Enimies.jsx, write src/data/image/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-image-json.mjs
 */
import fs from 'fs';
...
const imageDir = path.join(root, 'src', 'components', 'Image');
const dataDir = path.join(root, 'src', 'data', 'image');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/image/")) {
    c = c.replace(
...
for (const folder of franchises) {
  const base = path.join(imageDir, folder);
  ...
  const importLine = `import mdata from '../../../data/image/${jsonName}';`;
