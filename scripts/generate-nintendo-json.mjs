/**
 * Extract allies/enemies from Nintendo *Alies.jsx / *Enimies.jsx, write src/data/nintendo/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-nintendo-json.mjs
 */
import fs from 'fs';
...
const franchiseRoot = path.join(root, 'src', 'components', 'Nintendo');
const dataDir = path.join(root, 'src', 'data', 'nintendo');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/nintendo/")) {
    c = c.replace(
...
for (const folder of franchises) {
  ...
  const importLine = `import mdata from '../../../data/nintendo/${jsonName}';`;
