/**
 * Extract allies/enemies from Hasbro *Alies.jsx / *Enimies.jsx, write src/data/hasbro/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-hasbro-json.mjs
 */
import fs from 'fs';
...
const franchiseRoot = path.join(root, 'src', 'components', 'Hasbro');
const dataDir = path.join(root, 'src', 'data', 'hasbro');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/hasbro/")) {
    c = c.replace(
...
for (const folder of franchises) {
  ...
  const importLine = `import mdata from '../../../data/hasbro/${jsonName}';`;
