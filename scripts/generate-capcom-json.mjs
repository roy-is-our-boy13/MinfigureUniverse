/**
 * Extract allies/enemies from Capcom *Alies.jsx / *Enimies.jsx, write src/data/capcom/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-capcom-json.mjs
 */
import fs from 'fs';
...
const franchiseRoot = path.join(root, 'src', 'components', 'Capcom');
const dataDir = path.join(root, 'src', 'data', 'capcom');
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/capcom/")) {
    c = c.replace(
...
for (const folder of franchises) {
  ...
  const importLine = `import mdata from '../../../data/capcom/${jsonName}';`;
