/**
 * Extract allies/enemies from DC *Alies.jsx / *Enimies.jsx, write src/data/dc/*.json,
 * and update components to use CharacterPhotoGrid items={...}.
 * Run: node scripts/generate-dc-json.mjs
 */
import fs from 'fs';
...
function updateJsxFile(filePath, importLine, itemsExpr) {
  ...
  if (!c.includes("import mdata from '../../../data/dc/")) {
    c = c.replace(
...
const franchises = fs
  .readdirSync(dcDir, { withFileTypes: true })
  .filter((d) => d.isDirectory())
  .map((d) => d.name);

for (const folder of franchises) {
  const base = path.join(dcDir, folder);
  ...
  const importLine = `import mdata from '../../../data/dc/${jsonName}';`;
